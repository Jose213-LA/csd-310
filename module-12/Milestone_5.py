...
from tabulate import tabulate  # Import tabulate for clean output
...
def run_report(cursor, query, title):
    ...
    if results:
        headers = results[0].keys()  # Get column headers
        rows = [row.values() for row in results]  # Get each row's values
